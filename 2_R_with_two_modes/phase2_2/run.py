"""Plotting, artifact saving, and main experiment runner for Phase 2.2."""
...
def save_significance_md(
    ...
    lines = [
        f"# Phase 2.2 pilot paired check ({metric}, baseline = {baseline})", "",
    ...
    if num_seeds < 2:
        lines += [
            ...
            "Phase 2.2는 pilot이므로 통계 주장을 하지 않습니다. Phase 3.2 본 실험에서 seed를 늘려 해석합니다.",
        ]
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def save_classifier_report_md(report: dict, out_path: Path) -> None:
    ...
    lines = [
        "# Phase 2.2 — φ Classifier Evaluation (test set)", "",
...
def save_summary_md(
    ...
    lines = [
        "# Phase 2.2 MNIST Pipeline Pilot Summary", "",
        "## Configuration", "",
        f"- digits: {d0} vs {d1}",
        ...
        "## Pilot scope", "",
        "Phase 2.2는 schedule 성능 우위를 주장하는 실험이 아니라, Phase 3.2에서 사용할 "
        "empirical DMSR 계산, lambda_R* 추정, mixture sampler, diffusion 학습, DDIM 생성, "
        "평가 지표 저장 파이프라인이 끝까지 동작하는지 확인하는 pilot이다.",
        f"- mixture family: (1-eta)U[{config.lambda_min},{config.lambda_max}] + "
        f"eta TruncatedLaplace(center, beta)",
        f"- fixed eta values: {list(config.mixture_etas)}",
        f"- fixed beta values: {list(config.mixture_bs)}",
        "- intended configs: uniform, cosine_vp, mix_at0, mix_dmsr", "",
    ...
    lines += [
        ...
        "- 이 단계의 목적은 생성 성능 주장이 아니라 **파이프라인 검증**이다.",
        ...
        "- Phase 2.2에서는 eta/beta sweep을 하지 않는다. 같은 eta/beta에서 mix_at0와 "
        "mix_dmsr의 중심만 바꿔 sampler family와 artifact 저장 경로가 정상 동작하는지 본다.",
        ...
        "Phase 3.2에서 `--num-seeds 3` 이상으로 재실행하면 해석 가능한 paired 표가 채워진다.",
    ]
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run(config: ExperimentConfig, out_root: Path) -> Path:
    ...
    print(f"[Phase 2.2] device={config.device}  amp={config.amp}  compile={config.compile_model}")
    ...
    out_dir   = out_root / "phase2_2" / (base_name + "__RUNNING")
    ...
    try:
        ...
        print(f"[Phase 2.2] FAILED at stage '{meta['stage']}' -> {failed_dir}")
        raise
...
def _print_console_summary(config, dmsr_info, clf_report, agg_rows, sig_rows,
    ...
    d0, d1 = config.digits
    print("\n" + bar)
    print(f" PHASE 2.2 PILOT SUMMARY  |  MNIST digits {d0} vs {d1}  |  n_seeds={config.num_seeds}")
    print(bar)
...
def _run_body(config, out_dir, plots_dir, meta, _stage, _finalize) -> Path:
    ...
    print("[Phase 2.2] Training feature classifier phi...")
    ...
    print(f"[Phase 2.2] phi classifier: acc={clf_report['accuracy']*100:.2f}%  "
          f"macro-F1={clf_report['macro_f1']:.4f}")

    # 3. Compute empirical DMSR_φ(λ)
    _stage("compute_dmsr")
    print("[Phase 2.2] Computing empirical DMSR_phi(lambda)...")
    ...
    print(f"[Phase 2.2] lambda_R* = {lambda_r_star:.4f}  T_R = [{trans_low:.4f}, {trans_high:.4f}]")
    ...
    for seed_idx in range(config.num_seeds):
        ...
        for spec in specs:
            ...
            print(f"[Phase 2.2] Training {spec.name} (seed={run_seed})...")
            ...
            if config.n_generate > 0:
                ...
                print(f"[Phase 2.2]   Generating {config.n_generate} samples...")
            ...
            print(f"[Phase 2.2]   FID={phi_metrics['fid_phi']:.2f}  KID={phi_metrics['kid_phi']:.4f}  "
    ...
    print(f"[Phase 2.2] OK ({meta['duration_sec']}s) -> {final_dir}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run Phase 2.2 MNIST pipeline pilot.")
    p.add_argument("--digits",         type=int,   nargs=2, default=[0, 1])
    p.add_argument("--lambda-min",     type=float, default=-8.0)
    ...
    # Phase 3.2와 같은 mixture family. Phase 2.2 기본값은 sweep 없는 pilot.
    p.add_argument("--mixture-etas",   type=float, nargs="+", default=None,
                   help="mixture peak mass eta values. Phase 2.2 default: 0.5.")
    p.add_argument("--mixture-bs",     type=float, nargs="+", default=None,
                   help="truncated Laplace beta values. Phase 2.2 default: 2.0.")
    ...
    p.add_argument("--include-linear", dest="include_linear", action="store_true",
                   help="옵션: VP linear-β baseline schedule을 추가한다.")
    p.add_argument("--no-uniform",     dest="include_uniform", action="store_false",
                   help="uniform baseline schedule을 빼고 실행.")
    p.set_defaults(include_linear=False, include_uniform=True)
    ...
    p.add_argument("--run-name",       type=str,   default="phase2_2_mnist_pilot",
...
def main() -> None:
    ...
    print(f"[Phase 2.2] Using device: {device}")
    config = ExperimentConfig(
        ...
        mixture_etas=tuple(args.mixture_etas) if args.mixture_etas else (0.5,),
        mixture_bs=tuple(args.mixture_bs) if args.mixture_bs else (2.0,),
