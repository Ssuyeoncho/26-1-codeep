...
def train_one_schedule(
    ...
    for step in range(1, config.train_steps + 1):
        ...
        pred_eps = model(alpha * x0 + sigma * eps, lam.view(-1))
...
def compute_fid_phi(
    ...
    """FID in φ-feature space (no InceptionV3 download needed)."""
    ...
    return max(float(diff @ diff + np.trace(s1 + s2 - 2.0 * prod)), 0.0)
