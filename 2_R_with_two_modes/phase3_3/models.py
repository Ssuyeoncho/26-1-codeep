"""Utilities, data loading, and neural network models for Phase 3.3."""
...
from .config import CLASS_PAIRS, IMAGENETTE_CLASSES, ExperimentConfig
...
class TwoClassImageFolder(Dataset):
    def __init__(self, root: Path, class_a: str, class_b: str, transform) -> None:
        if not root.exists():
            raise FileNotFoundError(
                f"ImageNette split not found: {root}\n"
                "Expected data_root/imagenette2-160/{train,val}/<synset> after extraction."
            )
        syn_a, syn_b = IMAGENETTE_CLASSES[class_a], IMAGENETTE_CLASSES[class_b]
        self.dataset = datasets.ImageFolder(root, transform=transform)
        self.keep_class_indices = {self.dataset.class_to_idx[syn_a], self.dataset.class_to_idx[syn_b]}
        self.label_map = {
            self.dataset.class_to_idx[syn_a]: 0,
            self.dataset.class_to_idx[syn_b]: 1,
        }
        self.indices = [
            i for i, (_, label) in enumerate(self.dataset.samples)
            if label in self.keep_class_indices
        ]
...
def _ensure_imagenette(root: Path) -> Path:
    ds_root = root / "imagenette2-160"
    if (ds_root / "train").exists() and (ds_root / "val").exists():
        return ds_root
    archive = root / "imagenette2-160.tgz"
    root.mkdir(parents=True, exist_ok=True)
    if not archive.exists():
        print(f"[ImageNette] downloading {IMAGENETTE_URL} -> {archive}")
        urllib.request.urlretrieve(IMAGENETTE_URL, archive)
    print(f"[ImageNette] extracting {archive} -> {root}")
    with tarfile.open(archive, "r:gz") as tf:
        tf.extractall(root)
    if not (ds_root / "train").exists():
        raise RuntimeError(f"ImageNette extraction did not create expected directory: {ds_root}")
    return ds_root


def get_two_class_dataset(config: ExperimentConfig, train: bool) -> TwoClassImageFolder:
    ds_root = _ensure_imagenette(Path(config.data_root))
    ...
    return TwoClassImageFolder(ds_root / split, class_a, class_b, transform)
...
# ── U-Net (64×64, attention at 16×16 and bottleneck) ─────────────────────────

class UNet(nn.Module):
    """Small DDPM-style U-Net for 64×64 ImageNette. Epsilon-prediction."""
    ...
    def __init__(self, in_ch: int = 3, base_ch: int = 64, num_res_blocks: int = 2,
        ...
        self.enc1 = self._level(c0, c1, emb_dim, num_res_blocks, attn=False)  # 32×32
        self.down1 = Downsample(c1)
        self.enc2 = self._level(c1, c2, emb_dim, num_res_blocks, attn=True)   # 16×16
        ...
        self.dec1 = self._level(c2 + c1, c1, emb_dim, num_res_blocks + 1, attn=False)
