...
class ExperimentConfig:
    ...
    image_size: int = 32
    # Phase 3.2: 학습 λ 범위를 생성(DDIM) 범위와 '일치'시킨다([-8,8]).
    # 어떤 schedule이든 sampling trajectory 전체를 학습 support가 덮으므로, phase3에서
    # 좁은 집중 schedule이 생성 시작점(high-noise, λ≈-8)을 미학습으로 남겨 붕괴하던
    # support mismatch가 원천 차단된다. (DMSR λ_R* 계산 격자도 이 범위를 쓴다.)
    ...
    # ── Phase 3.2 핵심: mixture schedule 설계 ─────────────────────────────────
    #   p_train(λ) = (1-η)·Uniform[-8,8] + η·TruncatedLaplace_[-8,8](center, b)
    #   · Uniform floor  → 생성 trajectory 전체를 학습(coverage 보장, 붕괴 차단)
    #   · Laplace peak    → DMSR 가설(center=λ_R*) vs Hang(center=0) 강조를 검증
    #   floor가 커버를 책임지므로 b는 '작게'(sharp) 잡아야 emphasis가 눈에 보인다.
    #   중심만 {λ_R*, 0}로 바꿔 '중심 위치 효과'를 통제비교(η, b는 동일하게 유지).
    mixture_etas: tuple[float, ...] = (0.5,)   # peak 혼합비 η sweep
    mixture_bs: tuple[float, ...] = (1.0,)     # TruncatedLaplace peak scale b sweep
    include_center0: bool = True    # 중심 0(Hang-style) 대조군 포함 여부
    # 관행/대조 baseline. cosine_vp는 항상 포함(=유의성 검정 기준). uniform=η0 무정보 기준.
    include_linear: bool = False    # VP linear-β(DDPM) 유도분포 (기본 OFF; 필요시 켬)
    include_uniform: bool = True    # λ 범위 균일분포 (mixture η=0 특수형, 명시 baseline)
...
class ScheduleSpec:
    ...
    scale: float | None = None     # mixture kind에서는 Laplace peak scale b로 재사용
    eta: float | None = None       # mixture peak 혼합비 η (mixture kind 전용)
    note: str = ""
